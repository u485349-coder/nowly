...
import Animated, { Easing, Extrapolation, interpolate, interpolateColor, useAnimatedStyle, useSharedValue, withRepeat, withSpring, withTiming, } from "react-native-reanimated";
import { nowlyColors } from "../../constants/theme";
import { webPressableStyle } from "../../lib/web-pressable";
const AnimatedIcon = Animated.createAnimatedComponent(MaterialCommunityIcons);
export const FABToggle = ({ bottom, open, onPress, progress, icon = "plus", accentColor = nowlyColors.violet, size = 78, }) => {
    const pressProgress = useSharedValue(0);
    const breatheProgress = useSharedValue(0);
    const orbitProgress = useSharedValue(0);
    const logoBlendColor = nowlyColors.aqua;
    useEffect(() => {
        breatheProgress.value = withRepeat(withTiming(1, {
            duration: 2200,
            easing: Easing.inOut(Easing.sin),
        }), -1, true);
        orbitProgress.value = withRepeat(withTiming(1, {
            duration: 4200,
    ...
    return (_jsxs(Pressable, { accessibilityRole: "button", accessibilityState: { expanded: open }, hitSlop: 12, onPress: onPress, onPressIn: () => {
            ...
            webPressableStyle(pressed, { pressedOpacity: 0.96, pressedScale: 0.985 }),
                ...
                ], children: _jsx(View, { style: [styles.orbitDot, { backgroundColor: logoBlendColor }] }) }), _jsx(Animated.View, { pointerEvents: "none", style: [
                    styles.halo,
                    haloStyle,
                    {
                ...
                ], children: _jsxs(LinearGradient, { colors: ["#F3E8FF", nowlyColors.violet, logoBlendColor], end: { x: 1, y: 1 }, start: { x: 0.08, y: 0.08 }, style: [
                        styles.gradientFill,
                        {
                            width: size,
                            height: size,
                    ...
                    ], children: [_jsx(View, { style: styles.innerSheen }), _jsx(AnimatedIcon, { name: icon, size: 28, style: iconStyle })] }) })] }));
};
